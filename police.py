...
@police.route("/police_addcriminals", methods=['get','post'])
def police_addcriminals():
	data={}
	cid=request.args['cid']

	q="select * from  criminals where crime_num='%s'"%(cid)
	res=select(q)
	data['i']=res
	
	if 'criminal' in request.form:
		...
		q="INSERT INTO criminals VALUES(NULL,'%s','%s','%s','%s','%s')"%(cid,cname,gen,age,path)
		insert(q)
		flash("CRIME ADDED")
		return redirect(url_for("police.police_addcriminals",cid=cid))
	return render_template("police_addcriminals.html",data=data)
